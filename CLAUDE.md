...
`NinjaFigure3D`, `NinjaModel3D`, `ninjaPalette`), `fireninja/` (`FireNinjaHead`, `FireBladeFigure`, `FireNinjaFigure`,
`FireNinjaCelebration`, `FireNinjaFigure3D`, `FireNinjaModel3D`,
...
`LloydFigure3D`, `LloydModel3D`, `lloydPalette`),
`jettrooper/` (`JetTrooperHead`, `JetBlastFigure`, `JetTrooperFigure`,
`JetTrooperCelebration`, `JetTrooperFigure3D`, `JetTrooperModel3D`,
`jetTrooperPalette`),
...
`FigureStage3D` turntable — every avatar has both). A 3D model's moves are a
...
via the AA turret's `stepTurret`; a Bazooka Joe rocketeer **halts and kneels to
fire** — `soldierPlantHold` plants it a beat before each rocket and `StrikeRig`
freezes the patrol via `plantTimer`/`driftHold`, then the pure
`characters/shared/kneelStance` `stepKneel` driver drops it onto the knee),
four aim modes
