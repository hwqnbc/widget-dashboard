...
// reset-to-defaults would revert the gate count mid-lap — same guard
const resetDialog = () => page.getByRole('dialog').filter({ hasText: 'Reset settings?' })
const resetBtn = page.locator('[data-testid="dronesim-settings-reset"]')
await resetBtn.scrollIntoViewIfNeeded()
await resetBtn.click()
await page.waitForTimeout(300)
check('mid-lap settings reset asks for confirmation', (await resetDialog().count()) === 1)
await resetDialog().getByRole('button', { name: 'Keep course' }).click()
await page.waitForTimeout(300)
check('cancelled reset keeps 6 gates', (await gateCount()) === '6')
...
// with no best lap and no lap running, a settings reset that reverts the
// gate count applies without a confirmation
await openSettings(page)
await setGateSlider(0.999)
check('slider back to 6 (no guard, stats clear)', (await gateCount()) === '6')
const resetBtn2 = page.locator('[data-testid="dronesim-settings-reset"]')
await resetBtn2.scrollIntoViewIfNeeded()
await resetBtn2.click()
await page.waitForTimeout(400)
check('unguarded reset reverts to 3 gates', (await gateCount()) === '3')
await closeSettings(page)
