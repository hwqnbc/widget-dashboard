...
| `107-strike-gimbal` | gimballed weapon aiming: drag-to-aim slews `data-gimbal-yaw/-pitch` (reticle moves in Reticle mode, centred in Gunner), deep ground look-down + arc clamp, double-tap recenter, gimbal+assist kill with flight sticks idle, hover mode re-routes the right stick (drone holds position), `trackToward` soft-track dynamics (pure module), mode persistence |
