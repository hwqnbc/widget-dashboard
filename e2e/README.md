...
| `16-op-avatar` | operator = Player 1's avatar: default seat map renders the toy's `Model3D` (`data-op-avatar`/`data-op-figure` on the root), swapping Player 1 → DarkArin/frak/Imperium/Gold Gunner/Scar/Ninja on the real Settings page renders each avatar's model (all eight carry one — `data-op-figure` is always `avatar`; the basic primitive survives only as the Suspense fallback), swapping back restores toy; telemetry alive throughout |
...
| `100-strike-core` | Drone Strike: element presence + root defaults, damage-vignette + pad-chip at-rest contracts (`data-flash`/`data-low-hp`/`data-pad-state`), wave intro→active, seeded wave-1 composition vs the pure module (static balloon gallery + one slow road car), nearest-target beacon (balloon or car), fire-button tap + cooldown-limited hold, closed-loop engagement (aim onto the beacon, fire, target down, score/hits) |
...
| `109-strike-ground` | ground-target waves: pure-module ground trucks (`ground`) from wave 2 on the deck (low y, one-hit, difficulty-independent count), AA turrets (`turret`) from `TURRET_WAVE` (static, hp + shared return-fire gate follow the difficulty preset), and moving road cars (`car`) from `CAR_WAVE_START` (wave 1) — bound to a seeded road lane (fixed cross-coord ≈ a road `at`), wave-scaled travel velocity via `stepDrift`, difficulty-independent count, rendered as the `LegoSwatTruck` model (`CarTargets`); DOM clears wave 1 closed-loop and confirms wave 2 fields the seeded target count including the trucks (hit model is a normal sphere — covered by 100) |
...
| `121-avatars-3d` | Avatar Actions 2D/3D toggle + 3D action library: 2D default, switching to 3D lazy-loads three.js and renders a WebGL canvas for the avatars with a `Figure3D` (`data-view`, `data-figure3d`, `figure3d-stage`), the action toggle lists each model's `actions3d` moves (toy: Dance + 6 7 Show; ninja: Pump + Draw; fireninja: Fire Blade; darkarin: Twin Cross; frak: Blade Flurry; imperium: Claw Slash; goldgunner: Guns Blazing; scar: Breach & Clear) and drives `data-action`/`data-playing` (all eight avatars carry a 3D figure — the `figure3d-unavailable` placeholder path is scaffolding for future avatars, probed only by the toy block's negative check), tapping the figure toggles the turntable (`data-spin`, uniform across avatars, persisted), views restore on switch-back, chosen view + spin preference persist across reload |
...
| `140-model-viewer` | Model Viewer widget: adding it lazy-loads the shared three.js chunk and renders a WebGL canvas, default model is the LEGO SWAT truck (`data-model`), the render loop advances the tick-owned `data-frames` counter (frame production proven without pixels), Animate and Auto-rotate toggles drive `data-animate`/`data-autorotate`, the picker mirrors `data-model`, and both toggle settings + the model choice persist across reload |
