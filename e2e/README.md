...
| `16-op-avatar` | operator = Player 1's avatar: default seat map renders the toy's `Model3D` (`data-op-avatar`/`data-op-figure` on the root), swapping Player 1 → Ninja on the real Settings page falls back to the basic primitive figure, swapping back restores the model; telemetry alive throughout |
...
| `121-avatars-3d` | Avatar Actions 2D/3D toggle: 2D default, switching to 3D lazy-loads three.js and renders the toy's WebGL canvas (`data-view`, `data-figure3d`, `figure3d-stage`), the Idle/Celebrate toggle still works in 3D, an avatar without a `Figure3D` shows the `figure3d-unavailable` placeholder (no canvas, celebration toggle disabled), views restore on switch-back, chosen view persists across reload |
