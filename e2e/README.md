...
| `100-strike-core` | Drone Strike: element presence + root defaults, damage-vignette + pad-chip at-rest contracts (`data-flash`/`data-low-hp`/`data-pad-state`), wave intro→active, seeded wave-1 composition vs the pure module (static balloon gallery + moving road vehicles: a slow SWAT car and a military supply truck), nearest-target beacon (balloon/car/ground), fire-button tap + cooldown-limited hold, closed-loop engagement (aim onto the beacon, fire, target down, score/hits) |
