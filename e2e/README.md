...
| `16-op-avatar` | operator = Player 1's avatar: default seat map renders the toy's `Model3D` (`data-op-avatar`/`data-op-figure` on the root), swapping Player 1 → Fire Ninja on the real Settings page falls back to the basic primitive figure, Ninja renders its avatar model too, swapping back restores toy; telemetry alive throughout |
...
| `120-avatars` | Avatar Actions widget: default selection, every catalogued avatar selectable in order + renders a figure svg, the Idle/Celebrate toggle plays/stops the celebration (`data-playing`), switching avatar mid-play resets to the static figure, per-widget selection persists across reload while play state resets |
| `121-avatars-3d` | Avatar Actions 2D/3D toggle: 2D default, switching to 3D lazy-loads three.js and renders a WebGL canvas for the avatars with a `Figure3D` (toy, ninja) (`data-view`, `data-figure3d`, `figure3d-stage`), the Idle/Celebrate toggle still works in 3D, an avatar without one (fireninja) shows the `figure3d-unavailable` placeholder (no canvas, celebration toggle disabled), views restore on switch-back, chosen view persists across reload |
