...
| `130-map` | Map page: no arcgis code on the dashboard, the lazy chunk loads via the nav link, view readiness via `data-map-status` (from `view.when()`, never networkidle), basemap + injected ArcGIS CSS follow the theme toggle (render-computed `data-basemap`, asserts offline), 2D/3D toggle + reload persistence, tool strip contract, pure `routeGeometry` unit checks (bundled: insert index, nearest-distance, scale-derived tap threshold), deep link. OSRM routing is always mocked with an **echo mock** (line through the requested coords); an in-page CDN probe gates the online-only checks (view ready, attribution/zoom UI, click-driven pins with persistence + clear-all, waypoint editing: A→B distance, insert by clicking the line, remove by clicking a marker, undo of remove/insert/clear, measure widget mount/unmount) — on a CDN-blocked network the suite runs its offline branch |
