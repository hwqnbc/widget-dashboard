...
| `119-strike-soldiers` | rooftop-soldier targets: pure-module `soldier` targets appear from wave 1, stationed ON a building roof — each soldier's `(x,z)` is a building centre and its `y` is that roof's height + the torso offset (well above the deck's y≈1, its own roof each), static (no drift); hp follows difficulty (easy 1 / normal 2), the count is clamped to ≤2 (SoldierTargets draw-call budget), and they ride the shared return-fire gate (hold fire on wave 1 across every difficulty, armed by wave 5 on normal — like the AA turrets, via `stepTurret`). Weapon-matched **variants**: each soldier is variant 0 (rocket / Bazooka Joe) or 1 (SMG / Scar), assigned by order (wave-1 lone soldier is the rocketeer, a pair is one of each); the variant's weapon tags its projectile's `visual` (`SOLDIER_ROCKET`→rocket / `SOLDIER_SMG`→bolt, proven through `spawnProjectile`), so one enemy pool renders as both. DOM confirms the app fields the seeded wave-1 count with soldiers included. Rendered from the Scar / Bazooka Joe avatar `Model3D`s via SoldierTargets (which aim the weapon + play a firing pose); the live static soldier is cleared by 100/101/109 |
