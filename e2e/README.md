...
| `17-walker` | walking pilot: follow on foot (speed hard-capped, drone outruns it, follow-band idle) + full battery rescue (drain to dead → retrieve → carry → place on pad → recharge revives) via `data-op-*` |
