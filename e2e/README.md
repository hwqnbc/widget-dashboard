...
| `121-avatars-3d` | Avatar Actions 2D/3D toggle: 2D default, switching to 3D lazy-loads three.js and renders the toy's WebGL canvas (`data-view`, `data-figure3d`, `figure3d-stage`), tap play/stop still works in 3D, an avatar without a `Figure3D` shows the `figure3d-unavailable` placeholder (no canvas), views restore on switch-back, chosen view persists across reload |
