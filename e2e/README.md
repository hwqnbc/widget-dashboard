...
| `16-op-avatar` | operator = Player 1's avatar: default seat map renders the toy's `Model3D` (`data-op-avatar`/`data-op-figure` on the root), swapping Player 1 → DarkArin on the real Settings page falls back to the basic primitive figure, Ninja renders its avatar model too, swapping back restores toy; telemetry alive throughout |
