// The toy minifigure in 3D: the 2D ToyFigure's teal cap & torso, green legs
// and chest badge/shell rebuilt from three.js primitives on the shared
// turntable stage. Idle slowly rotates so the depth reads; `playing` runs the
// 3D take on the toy's "6 7" celebration — a bounce with both arms raised and
// pumping alternately. Animation mutates refs in useFrame (zero React
// renders), matching the drone widgets' input path.
//
...
function ToyModel({ playing }: { playing: boolean }) {
  ...
  useFrame((state, dt) => {
    ...
    if (spin) spin.rotation.y += dt * (playing ? 1.3 : 0.45)
